...
- 外生人气 proxy:
  - **审计结论：主线弃用**（Google Trends 覆盖不足且可比性存疑；Wikipedia Pageviews 当前仅覆盖 season 27）。
  - 因此 Q3 的主线解释模型仅使用官方数据与（可选）州人口等稳定参考变量；外生 proxy 只作为附录说明.
...
- 外生人气 proxy（Trends/Pageviews）主线弃用：报告中仅作为审计/附录失败尝试记录，不进入主线特征与结论.

## 10. 附录：深度学习对照实验（B，可失败，不进入主线）

目的：用现代 DL 做一条“对照线”，展示我们会用新方法与正确的失败诊断；主线结论仍以 Ridge/混合效应等可解释模型为准。

### 10.1 任务定义

- 回归目标：`yF = fan_vote_index`（来自 Q1 后验汇总）。
- 不确定性传播：从 Q1 后验抽样 K 次生成 K 份 `fan_vote_index`，重复训练/评估并汇总区间。

### 10.2 模型

- Baseline：Ridge / Lasso（或 statsmodels OLS）。
- 主线：混合效应（pro dancer random effect）。
- DL（附录）：小型 MLP 回归（tabular），可加 dropout/weight decay 作为过拟合对照。

### 10.3 评估

- 交叉验证：leave-one-season-out.
- 指标：RMSE/MAE + 置信区间覆盖率（对 K 次重复拟合的分布做汇总）。
- 失败分析写法：若 DL 泛化更差，展示训练-验证 gap、对超参/随机种子的敏感性，强调“小数据 + 强结构可解释问题”中传统/层级模型更合适.
...
- 输出文件：
  - `mcm2026c_q3_ml_fan_index_baselines_cv.csv`
