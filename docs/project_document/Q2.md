...
- Q1 后验依赖假设：报告不同 `tau`、不同先验强度等设置下的敏感性分析（主线不依赖外生人气 proxy）。
- 多淘汰/无淘汰周：在仿真中按观测规则强制处理，避免仿真路径与事实季结构冲突.

## 9. 工程落地命名（主线/对照/炫技）

- 主线：`src/mcm2026/pipelines/mcm2026c_q2_counterfactual_simulation.py`
- 对照（敏感性/案例复盘）：`src/mcm2026/pipelines/mcm2026c_q2_tau_sensitivity.py`, `src/mcm2026/pipelines/mcm2026c_q2_controversy_case_studies.py`
- 炫技（工具化 Agent，不作为主线输入）：`src/mcm2026/pipelines/showcase/mcm2026c_q2_agentic_ablation_runner.py`